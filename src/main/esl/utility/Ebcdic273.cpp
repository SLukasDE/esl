#include <esl/utility/Ebcdic273.h>

namespace esl {
inline namespace v1_6 {
namespace utility {

namespace {
/* Translates all of the standard Latin-1 characters from EBCDIC Code-Points 273 to standard Latin-1 Code-Points.
 * The remaining EBCDIC Code-Points are translated from Code-Points 037 charset to ISO 8859-1 character codes.
 * This translation is bidirectional.
 */
const std::uint32_t toLatin1Table[256] = {
		
};
}

std::string Ebcdic273::toLatin1(const std::string& str) {
	std::string result(str.size(), 0);

	for(std::string::size_type count = 0; count < str.size(); ++count) {
		result[count] = toLatin1Table[static_cast<unsigned char>(str[count])];
	}

	return result;
}

} /* namespace utility */
} /* inline namespace v1_6 */
} /* namespace esl */
